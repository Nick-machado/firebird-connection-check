...
export default function Clientes() {
  const { sector, canViewAllData, roleLabel, role, loading: roleLoading } = useUserRole();
  ...
  const clientesEquipeFiltrados = useMemo(() => {
    if (!clientesPermitidos) return [];
    const allowedEquipes = !canViewAllData && sector ? SECTOR_TO_EQUIPES[sector] : undefined;
    return filtrarClientesPorEquipe(clientesPermitidos, equipe, allowedEquipes);
  }, [clientesPermitidos, equipe, canViewAllData, sector]);

  const dadosProcessados = useMemo(() => {
    if (!clientesAPI) return null;

    // Classifica todos os clientes por status
    const statusData = classificarClientesPorStatus(clientesEquipeFiltrados);

    return statusData;
  }, [clientesAPI, clientesEquipeFiltrados]);
  ...
  return (
    <DashboardLayout>
      ...
      <ClienteVendasSheet
        ...
        categoriasFiltro={categoriasPermitidas}
      />
    </DashboardLayout>
  );
}
